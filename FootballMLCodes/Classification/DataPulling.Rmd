...
numeric_cols <- names(df_data)[sapply(df_data, is.numeric)]

# Prepare data for plotting density before and after imputation
plot_data_before <- melt(df_data[, numeric_cols])
plot_data_after <- melt(df_norm[, numeric_cols])

plot_data_before$Type <- "Raw Data"
plot_data_after$Type <- "Normalized"
...
ggplot(plot_data, aes(x = value, color = Type)) +
  ...
  labs(title = "Density Plot Before and After Normalization",
       x = "Value",
       y = "Density") +
  theme_minimal()
...
```{r}
##Plotting Before VS After Imputation
...
```{r}
write.csv(df_norm, "~/Desktop/Personal/Football ML.csv", row.names = FALSE) ##sending normalized data to desktop
write.csv(df_after, "~/Desktop/Personal/Football ML.csv", row.names = FALSE) ##sending normalized and imputed data to desktop
